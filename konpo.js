        ...
        if ($(window).width() >= 768) {
            ...
            allProjItems.each((idx, el) => {
                if (!$(el).find('[data-thumb-video]').hasClass('w-condition-invisible')) {
                    ...
                    let vidInner = $(`<video muted loop playsinline class="video-wrap"><source src=${$(el).find('[data-thumb-video]').text()} type="video/mp4"></video>`)
                    vidInner.appendTo(vidEl)
                    vidEl.appendTo($(el).find('.proj-gall-item-inner').eq(0))
                    $(el).find('.proj-gall-item-inner').eq(0).on('mouseover', function(e) {
                        $(this).find('.prj-vid').find('video').get(0).currentTime = 0
                        $(this).find('.prj-vid').find('video').get(0).play()
                    })
                    $(el).find('.proj-gall-item-inner').eq(0).on('mouseleave', function(e) {
                        $(this).find('.prj-vid').find('video').get(0).pause()
                    })
                }
            })
        }
